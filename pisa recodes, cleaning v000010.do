...
local pa "vigd modd paclass"
foreach var of local pa{
	gen `var'_5plus = 1 if `var' >= 5 & `var' < .
	replace `var'_5plus = 0 if `var' < 5
	}

local pa2 "paclass"
foreach var of local pa2{
                gen `var'_3plus = 1 if `var' >= 3 & `var' < .
                replace `var'_3plus = 0 if `var' < 3
                }
...
keep sex cntryid-bookid paclass-vigd vigd_5plus-paclass_5plus allpa ver_dat  ///
                                senwt pared-wvarstrr  homepos hisei homepos ///
                                wealth wealthq paclass_madj paclass_hours st061q01na paclass_3plus
